...
class HeartbeatPoller:
    def __init__(
        ...
        *,
        ...
        mcp_registry = None
        if hasattr(self.executor, "mcp_executor") and self.executor.mcp_executor:
            mcp_registry = self.executor.mcp_executor.registry

        self.source_poller = SourcePoller(
            config=self.profile if self.profile else {}, executor=self.executor, mcp_registry=mcp_registry
        )
