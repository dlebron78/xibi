...
class SubagentRetrieval:
    ...
    def get_recent_summaries(self, agent_id: str | None = None, limit: int = 5) -> list[dict[str, Any]]:
        """Get recent run summaries, optionally filtered by agent."""
        with open_db(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            query = (
                "SELECT id as run_id, agent_id, summary, completed_at, status, "
                "actual_cost_usd as cost_usd FROM subagent_runs WHERE status = 'DONE'"
            )
            params: list[Any] = []
            if agent_id:
                query += " AND agent_id = ?"
    ...
    def search_runs(self, query: str, agent_id: str | None = None) -> list[dict[str, Any]]:
