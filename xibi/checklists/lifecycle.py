...
def _handle_fire_recurrence(action_config: dict, ctx: ExecutionContext) -> HandlerResult:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        template_items = conn.execute("SELECT * FROM checklist_template_items WHERE template_id = ?", (template_id,)).fetchall()
                ...
                if not nudge_config.get("disable_warning_24h"):
                    action_ids.append(
                        register_action(
                            ...
                            trigger_config={"fire_at": (deadline_at - timedelta(hours=24)).strftime("%Y-%m-%dT%H:%M:%S")},
        ...
        if policy == "expire":
            ...
            for item in open_items:
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (str(uuid.uuid4()), new_instance_id, item["template_item_id"], item["label"], item["position"], None, None, "[]")
                )
            conn.execute("UPDATE checklist_instances SET status = 'expired', closed_at = ? WHERE id = ?", (now_str, prev_instance["id"]))
            ...
            for item in open_items:
                ...
                conn.execute(
                    "UPDATE checklist_instance_items SET rollover_prompted_at = ? WHERE id = ?",
                    (now_str, item["id"])
                )
...
def handle_rollover_callback(action: str, item_id: str, db_path: Path) -> str:
    ...
    with sqlite3.connect(db_path_str) as conn:
        ...
        new_instance = conn.execute(
            "SELECT id FROM checklist_instances WHERE template_id = ? ORDER BY created_at DESC LIMIT 1",
            (template_id,)
        ).fetchone()
        ...
        if action == "done":
            ...
            if new_instance and new_instance["id"] != instance["id"]:
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (str(uuid.uuid4()), new_instance["id"], item["template_item_id"], item["label"], item["position"], None, None, "[]")
                )
                conn.execute("UPDATE checklist_instance_items SET rollover_prompted_at = NULL WHERE id = ?", (item_id,))
