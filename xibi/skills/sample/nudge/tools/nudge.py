...
def run(params: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if not token:
            env_path = (
                workdir.parent / ".xibi_env" if (workdir.parent / ".xibi_env").exists() else Path.home() / ".xibi_env"
            )
            if env_path.exists():
        ...
        # Attempt to use TelegramAdapter for consistency, but have a raw urllib fallback
        try:
