...
- [P2] [feature] **Semantic memory recall (sqlite-vec)** — `_get_session_memories()` and the `recall` tool currently use substring/keyword matching against the beliefs table. Add sqlite-vec to the db connection and generate embeddings on `remember()` writes; recall becomes a vector similarity search. This closes the gap with Mem0/Zep-style systems without replacing the local SQLite memory layer or losing the session integration. Contained change: new `belief_embeddings` shadow table, updated `xibi/db/migrations.py`, updated `recall.py`. Fallback to keyword if vec extension unavailable. Does NOT require MCP foundation work — can queue independently after step-35.
