...
| Email write idempotency | 2 | DONE (step-133) | `_process_email_signals` write phase uses one SQLite transaction per email (`with xibi.db.open_db(self.db_path) as conn:`, single context manager) instead of one batch transaction, so a single email's DB failure no longer rolls back `mark_seen`/`triage_log` for emails that already processed (the old behavior re-fired stale nudges next tick). Nudge broadcast moved outside the per-email transaction (SQLite lock released before Telegram I/O) but kept inside the per-email `try`, so a send failure is isolated to that email and cannot abort the batch. Vestigial `or item["verdict"] == "DEFER"` write-gate arm removed (DEFER absent from `VALID_TIERS`). Age gate (`XIBI_EMAIL_MAX_AGE_DAYS`, default 7) now owned solely by the poller; the `list_unread.py` hotfix filter was removed. Tests: `tests/test_email_age_gate.py` (per-email isolation, nudge-outside-tx, DEFER removal). |
...
| step-135 | Slim ReAct system-prompt mode (stop conversational over-tooling) | DONE 2026-06-23. `xibi/react.py` adds a `slim` prompt variant behind `XIBI_REACT_PROMPT_MODE` (env, not the immutable config.json; unrecognized→`heavy`+WARNING). `slim` drops the identity "act with initiative" line and replaces Rule 1 "LOOK BEFORE YOU LEAP" with a `WHEN TO USE TOOLS` block so the small local model stops treating conversational statements as research tasks. Rules 2-5, `DELIMITER_INSTRUCTION`, and drafts/context/tools/handle blocks are byte-identical across modes (verified vs pre-change HEAD; assembly extracted to `_assemble_system_prompt` + `_identity_lines_for_mode` + `_rules_for_mode`, with `_build_tools_block`/`_HANDLE_INSTRUCTIONS` reused by the canary harness). `prompt_mode` resolved once above the span closure, tagged on the `react.run` span, logged once per turn. Default ships `heavy` (no behavior change); flip is env-only via `~/.xibi/secrets.env`, reversible in one command. Scope = interactive chat only (`react.run` callers: telegram + cli/chat; heartbeat extraction/classification untouched). Tests: `tests/test_prompt_slim.py` (env resolution, assembly byte-identity, span attr — CI) + a `live` fixture corpus; canary `scripts/prompt_slim_eval.py`. Supersedes step-134 (parked NOT READY). |
| step-140 | Stop reporting total search-engine failure as success; repoint manifest | DONE 2026-09-20. Verified against merged code, not the spec. `skills/search/tools/search_searxng.py:52` returns `status="error"` naming the dead engines only when `results` is empty AND `unresponsive_engines` is non-empty; the genuine-empty branch (`:67` `"No results found."`) is unchanged, which is the distinction the step exists for. Module logger at `:15` emits `search.engines_unavailable` at `:53` — `logger.warning`, never `logging.warning` (the latter would `basicConfig()` a root StreamHandler into `xibi-telegram`, which configures no logging of its own). No alternative tool is named in Python: provider preference lives only in the three rewritten `skills/search/manifest.json` descriptions (`search_tavily`, `search_searxng`, `read_page`), and `read_page.py`'s docstring is now search-agnostic. `control_plane_triggers` and `"access": "operator"` left untouched — both verified inert (nothing under `xibi/` reads either key; `xibi/tools.py:18` `DEFAULT_TIER = GREEN`). SearXNG container and `settings.yml` untouched per Constraints. Tests: `tests/test_search_tools.py`, 11 tests. Opus review mutation-tested 11 wrong implementations, 10 caught; the survivor (deleted `logger.warning`) closed in `55fddf7`. **Open:** the "prefer search_tavily" wording is an outage-driven preference with no automatic expiry — revisit when the SearXNG engine set recovers. PDV runs post-deploy per PR #152. |
