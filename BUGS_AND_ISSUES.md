...
| BUG-013 | **`extract_signals_llm` missing `config_path` passthrough** | 2026-05-13 | 🟢 Fixed | Introduced in step-128 (commit `9fbf9a3`). `xibi/heartbeat/llm_extractor.py:208` called `get_model(effort="fast")` without `config_path`, while every other heartbeat caller passes `config_path=self.config_path` (poller.py:196, classification.py, rich_nudge.py). Result: the LLM extractor resolved its model against `get_model`'s built-in default (`"config.json"`) instead of the operator-configured path, so its fallback chain could diverge from the rest of the heartbeat. Fix: added `config_path: str \| None = None` kwarg to `extract_signals_llm`, plumbed through from poller call site, and removed the `del context` discard so the parameter is preserved for future spec hooks. Fixed in hotfix `hotfix/bug-013-014-llm-extractor-config-path-and-shadow-metric`. |
| BUG-014 | **Shadow comparison runs after email Tier 0 merge** | 2026-05-13 | 🟢 Fixed | Introduced in step-128 (commit `9fbf9a3`). `xibi/heartbeat/poller.py:606` called `compare_extractions` *after* `merge_email_tier0_signals` at line 605. When the LLM returned an empty list, the merge returned the coded signals unchanged (positional pairing with all `partner=None`), so the comparison saw two identical lists and reported a false 100% match — `ref_id_matches == coded_count`, `count_ratio == 1.0`, `field_coverage_pct == 100`. Shadow data was unusable for "is the LLM producing equivalent output?" because the metric collapsed to "do coded signals equal themselves?". Fix: moved `compare_extractions` *before* the merge so the metric reflects raw LLM output quality. The merge still runs after for the in-`llm`-mode pipeline swap. Fixed in hotfix `hotfix/bug-013-014-llm-extractor-config-path-and-shadow-metric`. |
