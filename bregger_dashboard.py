...
def api_subagent_summary():
    try:
        with get_db() as conn:
            active = conn.execute("SELECT COUNT(*) FROM subagent_runs WHERE status IN ('SPAWNED', 'RUNNING')").fetchone()[0]
...
def api_subagent_runs():
    try:
        ...
        with get_db() as conn:
            cursor = conn.execute(
                "SELECT * FROM subagent_runs ORDER BY created_at DESC LIMIT ?", (limit,)
            )
...
def api_subagent_cost_breakdown():
    try:
        with get_db() as conn:
            ...
            return jsonify({
                "per_agent": [dict(row) for row in agent_costs],
                "per_model": [dict(row) for row in model_costs]
            })
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
