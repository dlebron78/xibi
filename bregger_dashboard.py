...
app = Flask(__name__)
DB_PATH = os.path.join(os.environ.get("XIBI_DATA_DIR", os.path.join(os.path.expanduser("~"), "bregger_remote")), "data", "bregger.db")
    ...
    try:
        with get_db() as conn:
            ...
            if row and row['model']:
                ...
                try:
                    config_path = os.path.join(os.environ.get("XIBI_DATA_DIR", os.path.join(os.path.expanduser("~"), "bregger_remote")), "config.json")
            ...
            bot_alive = any("bregger" in p.name().lower() or "bregger" in " ".join(p.cmdline()).lower() 
                            for p in psutil.process_iter(['name', 'cmdline']) if p.pid != psutil.Process().pid)
        ...
        with get_db() as conn:
            ...
            try:
                ...
                for row in cursor.fetchall():
                    ...
                    existing = next((t for t in active_threads if normalize_topic(t["topic"]) == normalize_topic(row["topic"])), None)
                    if existing:
            ...
            final_threads = pinned + [t for t in active_threads if not t.get("pinned", False) or t in pinned == False] # keep existing pinned
