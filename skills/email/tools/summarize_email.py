...
def run(params: Dict):
            ...
            for part in msg.walk():
                if part.get_content_type() == "text/plain" and not body:
                    text = (
                        ...
                        .decode(part.get_content_charset("utf-8") or "utf-8", errors="replace")
                        .strip()
                    )
                    if text.lower() not in ("textual email", "text email"):
                        body = text
    ...
    if multiple_provided:
        msg_str += (
            ". NOTE: I only read the FIRST email. To read the others, call summarize_email again with the next ID."
        )
