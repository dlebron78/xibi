...
def run(params):
    ...
    if not all([email, imap_host, smtp_host, password]):
        return {
            "status": "error",
            "message": "Missing required configuration parameters (email, imap_host, smtp_host, password).",
        }
...
"""
    ...
    try:
        ...
        return {"status": "success", "message": f"Successfully configured email for {email}. Permissions set to 600."}
