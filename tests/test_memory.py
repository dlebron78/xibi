...
from skills.memory.tools import remember, recall, archive
...
def test_recall_filters_expired(fresh_db):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        conn.execute("INSERT INTO ledger (id, category, content, status) VALUES ('id2', 'note', 'Expired note', 'expired')")
        conn.commit()
...
def test_get_active_threads_pure_sql(fresh_db):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        for _ in range(2):
            conn.execute("INSERT INTO signals (id, topic_hint, timestamp) VALUES (lower(hex(randomblob(4))), 'Topic_C', datetime('now', '-10 days'))")
        conn.commit()
