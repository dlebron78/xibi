...
def test_email_context_alias():
    ctx = EmailContext(signal_ref_id="sig123", sender_id="user@example.com", sender_name="User", headline="Hello")
    assert isinstance(ctx, SignalContext)


def test_old_field_names_via_property():
    ctx = SignalContext(signal_ref_id="sig123", sender_id="user@example.com", sender_name="User", headline="Hello")
...
def test_source_channel_field():
    ctx = SignalContext(signal_ref_id="1", sender_id="2", sender_name="3", headline="4", source_channel="calendar")
