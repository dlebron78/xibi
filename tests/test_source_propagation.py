...
from xibi.react import dispatch, run
...
def test_dispatch_passes_source_to_check():
    ...
    mock_command_layer.check.return_value = CommandResult(
        allowed=True, tier="green", validation_errors=[],
        dedup_suppressed=False, audit_required=False,
        block_reason="", retry_hint="", source_bumped=False
    )

    dispatch(
        "list_emails", {"query": "test"}, [],
        command_layer=mock_command_layer,
        prev_step_source="mcp:gmail"
    )
