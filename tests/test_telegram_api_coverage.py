...
def test_get_telegram_config_allowed_ids():
    with patch.dict(os.environ, {"TELEGRAM_BOT_TOKEN": "token123", "XIBI_TELEGRAM_ALLOWED_CHAT_IDS": "111,222"}):
        with patch.dict(os.environ, {}, clear=True):
            # need to be careful with clear=True, it might remove too much
            pass
        # manually clear the ones we don't want
        if "TELEGRAM_CHAT_ID" in os.environ: del os.environ["TELEGRAM_CHAT_ID"]
        if "XIBI_TELEGRAM_CHAT_ID" in os.environ: del os.environ["XIBI_TELEGRAM_CHAT_ID"]

        token, chat_id = api._get_telegram_config()
        assert chat_id == 111

def test_send_nudge_no_config():
    with patch("xibi.telegram.api._get_telegram_config", return_value=(None, None)):
        with patch("xibi.telegram.api.logger.error") as mock_log:
            api.send_nudge("hello")
            mock_log.assert_called_with("Telegram not configured for nudge")

def test_send_nudge_success():
    with patch("xibi.telegram.api._get_telegram_config", return_value=("token", 123)):
        with patch("xibi.telegram.api._api_call") as mock_call:
