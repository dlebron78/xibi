...
def setup_observation_cycles_table(conn):
    conn.execute("""
        CREATE TABLE observation_cycles (
            ...
            error_log TEXT,
            review_mode TEXT DEFAULT 'triage'
        )
    """)
...
def test_get_signal_pipeline_counts_by_urgency(conn):
    ...
    now = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
    conn.execute(
        "INSERT INTO signals (source, urgency, timestamp, content_preview) VALUES ('s1', 'high', ?, 'p1')", (now,)
    )
    conn.execute(
        "INSERT INTO signals (source, urgency, timestamp, content_preview) VALUES ('s2', 'high', ?, 'p2')", (now,)
    )
    conn.execute(
        "INSERT INTO signals (source, urgency, timestamp, content_preview) VALUES ('s3', 'normal', ?, 'p3')", (now,)
    )
...
def test_get_active_threads_fields(conn):
    setup_threads_table(conn)
    conn.execute(
        "INSERT INTO threads (id, name, status, owner, signal_count) VALUES ('t1', 'Job search', 'active', 'them', 5)"
    )
...
def test_observation_error_log_written_on_degraded_cycle(tmp_path):
    db_path = tmp_path / "xibi.db"
    conn = sqlite3.connect(db_path)
    setup_observation_cycles_table(conn)
    conn.execute(
        "CREATE TABLE signals (id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP, source TEXT NOT NULL, content_preview TEXT NOT NULL)"
    )
    ...
    conn.execute(
        "CREATE TABLE tasks (id TEXT PRIMARY KEY, goal TEXT, status TEXT, urgency TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)"
    )
    conn.execute(
        "CREATE TABLE beliefs (key TEXT PRIMARY KEY, value TEXT, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP)"
    )
    conn.commit()
    conn.close()
    ...
    cycle.run(executor=executor, command_layer=command_layer)

    # Verify error_log in DB
    conn = sqlite3.connect(db_path)
    row = conn.execute(
        "SELECT error_log, degraded, role_used FROM observation_cycles ORDER BY id DESC LIMIT 1"
    ).fetchone()
    conn.close()
...
def test_observation_error_log_empty_on_success(tmp_path):
    db_path = tmp_path / "xibi.db"
    conn = sqlite3.connect(db_path)
    setup_observation_cycles_table(conn)
    conn.execute(
