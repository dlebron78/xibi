...
def test_span_has_system_prompt_preview(db_path, config):
    ...
    try:
        with patch("requests.post") as mock_post:
            mock_post.return_value.json.return_value = {"response": "ok"}
            mock_post.return_value.status_code = 200
            client.generate("hi", system="You are a helpful assistant who likes to talk a lot and has many things to say.")
    finally:
        clear_trace_context()
...
def test_parse_recovery_updates_parse_status(db_path, config):
    ...
    responses = [
        "BAD JSON",
        "{\"thought\": \"done\", \"tool\": \"finish\", \"tool_input\": {\"answer\": \"recovered\"}}"
    ]
...
def test_full_waterfall_llm_then_tool_then_llm(db_path, config):
    ...
    with open_db(db_path) as conn:
        ...
        rows = conn.execute("SELECT operation, start_ms FROM spans WHERE trace_id IS NOT NULL AND operation != 'react.run' ORDER BY start_ms").fetchall()
