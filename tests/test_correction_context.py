...
from xibi.heartbeat.classification import query_correction_context, build_classification_prompt, build_fallback_prompt
...
def test_query_sender_match(db_path):
    ...
    conn.execute("INSERT INTO signals (ref_id, urgency, sender_contact_id, topic_hint) VALUES ('msg1', 'HIGH', 'c1', 'topic1')")
    # Another correction for same sender
    conn.execute("INSERT INTO triage_log (email_id, verdict) VALUES ('msg2', 'LOW')")
    conn.execute("INSERT INTO signals (ref_id, urgency, sender_contact_id, topic_hint) VALUES ('msg2', 'HIGH', 'c1', 'topic1')")
    conn.commit()
    conn.close()
...
def test_query_topic_match(db_path):
    ...
    conn.execute("INSERT INTO signals (ref_id, urgency, sender_contact_id, topic_hint) VALUES ('msg2', 'HIGH', 'c2', 'topic1')")
    conn.commit()
    conn.close()
...
def test_query_lookback_window(db_path):
    conn = sqlite3.connect(db_path)
    # Old correction
    conn.execute("INSERT INTO triage_log (email_id, verdict, timestamp) VALUES ('old', 'LOW', datetime('now', '-31 days'))")
    conn.execute("INSERT INTO signals (ref_id, urgency, sender_contact_id, topic_hint, timestamp) VALUES ('old', 'HIGH', 'c1', 'topic1', datetime('now', '-31 days'))")
    # Recent correction
    conn.execute("INSERT INTO triage_log (email_id, verdict) VALUES ('new', 'LOW')")
    conn.execute("INSERT INTO signals (ref_id, urgency, sender_contact_id, topic_hint) VALUES ('new', 'HIGH', 'c1', 'topic1')")
    conn.commit()
    conn.close()
...
def test_query_limit_5(db_path):
    ...
    for i in range(10):
        ...
        conn.execute(f"INSERT INTO signals (ref_id, urgency, sender_contact_id, topic_hint) VALUES ('msg{i}', 'HIGH', 'c{i}', 'topic1')")
    conn.commit()
    conn.close()
...
def test_query_correction_reason_included(db_path):
    conn = sqlite3.connect(db_path)
    conn.execute("INSERT INTO triage_log (email_id, verdict) VALUES ('msg1', 'LOW')")
    conn.execute("INSERT INTO signals (ref_id, urgency, sender_contact_id, topic_hint, correction_reason) VALUES ('msg1', 'HIGH', 'c1', 'topic1', 'Should be high')")
    conn.commit()
    conn.close()
...
def test_prompt_includes_corrections(db_path, monkeypatch):
    monkeypatch.setattr("xibi.heartbeat.classification.query_correction_context", lambda **kwargs: [
        {'original_tier': 'LOW', 'corrected_tier': 'HIGH', 'topic_hint': 'invoice', 'sender_contact_id': 'c1', 'correction_count': 3, 'latest_reason': 'Business critical'}
    ])
...
def test_prompt_includes_manager_reason(db_path, monkeypatch):
    monkeypatch.setattr("xibi.heartbeat.classification.query_correction_context", lambda **kwargs: [
        {'original_tier': 'LOW', 'corrected_tier': 'HIGH', 'topic_hint': 'invoice', 'sender_contact_id': 'c1', 'correction_count': 1, 'latest_reason': 'Reason X'}
    ])
    ctx = SignalContext(signal_ref_id="m", sender_id="s", sender_name="n", headline="h", db_path=db_path, contact_id="c1")
    prompt = build_classification_prompt({}, ctx)
    assert "Manager noted: \"Reason X\"" in prompt

def test_prompt_omits_null_reason(db_path, monkeypatch):
    monkeypatch.setattr("xibi.heartbeat.classification.query_correction_context", lambda **kwargs: [
        {'original_tier': 'LOW', 'corrected_tier': 'HIGH', 'topic_hint': 'invoice', 'sender_contact_id': 'c1', 'correction_count': 1, 'latest_reason': None}
    ])
    ctx = SignalContext(signal_ref_id="m", sender_id="s", sender_name="n", headline="h", db_path=db_path, contact_id="c1")
    prompt = build_classification_prompt({}, ctx)
...
def test_prompt_correction_count_shown(db_path, monkeypatch):
    monkeypatch.setattr("xibi.heartbeat.classification.query_correction_context", lambda **kwargs: [
        {'original_tier': 'LOW', 'corrected_tier': 'HIGH', 'topic_hint': 'invoice', 'sender_contact_id': 'c1', 'correction_count': 42, 'latest_reason': None}
    ])
    ctx = SignalContext(signal_ref_id="m", sender_id="s", sender_name="n", headline="h", db_path=db_path, contact_id="c1")
    prompt = build_classification_prompt({}, ctx)
...
def test_classify_signal_with_corrections(db_path):
    ...
    conn.execute("INSERT INTO signals (ref_id, urgency, sender_contact_id, topic_hint, correction_reason) VALUES ('prev', 'HIGH', 'c1', 'topic1', 'Important topic')")
    conn.commit()
    conn.close()
