...
def test_all_local_skill_manifests_have_input_schema():
    ...
    for manifest_path in skills_dir.glob("*/manifest.json"):
        ...
        for tool in manifest.get("tools", []):
            ...
            assert isinstance(tool["inputSchema"], (dict, list)), (
                f"inputSchema in {tool.get('name')} in {manifest_path} should be dict or list"
            )
...
def test_react_loop_validates_tool_input():
    ...
    skill_registry = [{"name": "test_tool", "inputSchema": {"path": {"type": "string"}}}]
    ...
    with patch("xibi.react.get_model", return_value=mock_model):
        ...
        result = run(
            query="test", config=config, skill_registry=skill_registry, command_layer=cl, executor=MockExecutor()
        )
