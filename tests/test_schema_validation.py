...
def test_all_local_skill_manifests_have_inputSchema():
    ...
    for manifest_path in skills_dir.glob("*/manifest.json"):
        ...
        for tool in manifest.get("tools", []):
            ...
            assert isinstance(tool["inputSchema"], dict), f"'inputSchema' for tool '{tool.get('name')}' in {manifest_path} must be a dict"
            assert tool["inputSchema"].get("type") == "object", f"'inputSchema' for tool '{tool.get('name')}' in {manifest_path} must have type 'object'"

def test_react_loop_validates_tool_input():
    config = {
        "models": {
            "text": {
                "fast": {"provider": "mock", "model": "mock-model"}
            }
        }
    }
    skill_registry = [
