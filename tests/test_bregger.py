...
class TestExecutive:
    ...
    def test_workdir_injected_into_params(self, tmp_path):
        ...
        plan = {
            "skill": "memory",
            "tool": "remember",
            "parameters": {"content": "test injection"}
        }
...
class TestWorkingMemory:
    def test_table_creation(self, tmp_workdir):
        ...
        with sqlite3.connect(c.db_path) as conn:
            rows = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='conversation_history'").fetchall()
        assert len(rows) == 1
...
class TestConversationRecall:
    ...
    def test_recall_keyword_match(self, core):
        ...
        with sqlite3.connect(core.db_path) as conn:
            conn.execute("INSERT INTO conversation_history (user_message, bot_response, created_at) VALUES ('hello apple', 'hi', '2024-01-01 10:00:00')")
            conn.execute("INSERT INTO conversation_history (user_message, bot_response, created_at) VALUES ('hello banana', 'hi', '2024-01-02 10:00:00')")
    ...
    def test_recall_limit(self, core):
        ...
        with sqlite3.connect(core.db_path) as conn:
            for i in range(15):
                conn.execute(f"INSERT INTO conversation_history (user_message, bot_response, created_at) VALUES ('test orange {i}', 'hi', '2024-01-01 10:00:00')")
                
        res = recall_conversation.run({"query": "orange", "limit": 2, "_workdir": core.db_path.parent.parent.as_posix()})
        assert res["status"] == "success"
...
class TestReActLoop:
    ...
    def test_step_full_text_contains_fields(self):
        from bregger_core import Step
        s = Step(step_num=2, thought="I need to read the file", tool="read_file",
    ...
    def test_step_one_line_summary_error(self):
        from bregger_core import Step
        s = Step(step_num=3, thought="", tool="send_email", tool_input={"to": "bad"},
    ...
    def test_react_force_finish_on_max_steps(self, core):
        ...
        steps = [
            Step(i, "still trying", "nonexistent_tool", {"x": i})
            for i in range(1, 10)
        ]
    ...
    def test_react_finish_step_returns_answer(self, core):
        ...
        core.router.set_step_sequence([
            Step(1, "I know the answer.", "finish",
...
class TestUnifiedMemorySignals:
    def test_chat_signals_logged(self, core):
        ...
        core.router.set_step_sequence([
            Step(1, "I will check the schedule.", "finish", {"final_answer": "Done"}, signal={"topic": "scheduling", "entity": "calendar", "entity_type": "other"}),
        ])
        core.process_query("check my calendar")

        with sqlite3.connect(core.db_path) as conn:
            row = conn.execute("SELECT source, topic_hint, entity_text FROM signals WHERE ref_source='traces'").fetchone()
    ...
    def test_cross_channel_escalation(self, core):
        ...
        with sqlite3.connect(core.db_path) as conn:
            conn.execute("INSERT INTO signals (source, topic_hint, timestamp, content_preview) VALUES ('chat', 'presentation_deck', datetime('now', '-1 hours'), 'test1')")
            conn.execute("INSERT INTO signals (source, topic_hint, timestamp, content_preview) VALUES ('chat', 'presentation_deck', datetime('now', '-2 hours'), 'test2')")
            conn.commit()
    ...
    def test_active_threads_merged(self, core):
        ...
        with sqlite3.connect(core.db_path) as conn:
            conn.execute("INSERT INTO signals (source, topic_hint, timestamp, content_preview) VALUES ('chat', 'scheduling', datetime('now', '-1 hours'), 'test1')")
            conn.execute("INSERT INTO signals (source, topic_hint, timestamp, content_preview) VALUES ('chat', 'calendar', datetime('now', '-2 hours'), 'test2')")
            conn.execute("INSERT INTO signals (source, topic_hint, timestamp, content_preview) VALUES ('chat', 'my schedule', datetime('now', '-3 hours'), 'test3')")
            conn.commit()
    ...
    def test_skill_triggers_auto_registered(self, tmp_path):
        ...
        manifest = {
            ...
            "control_plane_triggers": [
                {"regex": "^trigger me$", "intent": "test_intent", "tool": "test_tool"}
            ],
