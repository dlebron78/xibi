...
def test_recall_reads_from_beliefs_table(xibi_workdir):
    ...
    found = [item for item in res["items"] if item.get("source") == "belief" and "belief_keyword" in item.get("key", "")]
    assert len(found) > 0
...
def test_recall_reads_from_ledger_table(xibi_workdir):
    ...
    with sqlite3.connect(db_path) as conn:
        conn.execute(
            "INSERT INTO ledger (id, category, content) VALUES (?, ?, ?)",
            ("test-id", "note", "ledger_keyword content")
        )
        conn.commit()
    ...
    found = [item for item in res["items"] if item.get("source") == "ledger" and "ledger_keyword" in item.get("content", "")]
    assert len(found) > 0
...
def test_recall_merges_both_sources(xibi_workdir):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        conn.execute(
            "INSERT INTO ledger (id, category, content) VALUES (?, ?, ?)",
            ("id-2", "note", "Ledger shared_key content")
        )
        conn.commit()
...
def test_archive_expires_belief(xibi_workdir):
    ...
    remember.run({"content": "to be archived", "category": "preference", "entity": "test_key", "_workdir": str(xibi_workdir)})
...
def test_remember_missing_db_raises_cleanly():
    ...
    assert "Database not found" in res["message"] or "No such file or directory" in res["message"] or "unable to open database file" in res["message"]
