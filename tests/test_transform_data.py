...
def test_group_by_returns_dict_of_lists():
    ...
    params = {"handle": payload, "operations": [{"op": "group_by", "args": {"field": "team"}}]}
    result = transform_data.run(params)
...
def test_dedupe():
    ...
    params = {"handle": payload, "operations": [{"op": "dedupe", "args": {"field": "id"}}]}
    result = transform_data.run(params)
    assert len(result) == 2
...
def test_project():
    payload = [{"a": 1, "b": 2, "c": 3}]
    params = {"handle": payload, "operations": [{"op": "project", "args": {"fields": ["a", "c"]}}]}
    result = transform_data.run(params)
...
def test_transformation_error():
    ...
    payload = [{"a": 1}]
    params = {"handle": payload, "operations": [{"op": "nope_not_a_real_op", "args": {}}]}
    result = transform_data.run(params)
...
def test_filter_missing_field_is_lenient():
    ...
    payload = [{"a": 1}, {"b": 2}, {"a": 5}]
    params = {"handle": payload, "operations": [{"op": "filter", "args": {"field": "a", "op": ">", "value": 0}}]}
    result = transform_data.run(params)
...
def test_sort_with_missing_and_none_fields():
    ...
    params = {"handle": payload, "operations": [{"op": "sort", "args": {"field": "s", "order": "asc"}}]}
    result = transform_data.run(params)
...
def test_sort_with_heterogeneous_types():
    ...
    params = {"handle": payload, "operations": [{"op": "sort", "args": {"field": "v", "order": "asc"}}]}
    result = transform_data.run(params)
...
def test_write_file_resolves_handle(tmp_path):
    ...
    params = {"filepath": filepath, "handle": payload, "_workdir": str(tmp_path)}
