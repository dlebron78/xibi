...
def test_session_start_shows_blocked_actions(tmp_path):
    ...
    with open_db(db_path) as conn, conn:
        conn.execute(
            "CREATE TABLE access_log (chat_id TEXT, authorized INTEGER, user_name TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP, prev_step_source TEXT, source_bumped INTEGER, effective_tier TEXT)"
        )
